...
cb_reject =      [ 'set( findobj(''parent'', gcbf, ''tag'', ''rejstatus''), ''value'', 3);' ... % force status to 3 
				    checkstatus ...
				   '[EEG LASTCOM] = eeg_rejsuperpose(EEG,' int2str(icacomp) ',1,1,1,1,1,1,1); h(LASTCOM, EEG);' ...
				   '[EEG LASTCOM] = pop_rejepoch( EEG, EEG.reject.rejglobal, 1);' ...
				   'if ~isempty(LASTCOM), ' ...
				   '   h(LASTCOM, EEG); [ALLEEG EEG CURRENTSET LASTCOM] = pop_newset(ALLEEG, EEG, CURRENTSET); h(LASTCOM);' ...
...
lisboxoptions = { 'string', [ 'Show only the new trials marked for rejection by the measure selected above|' ...
	...
	...
	{ }, { 'Style', 'text', 'string', ['Max slope ' fastif(icacomp, '(uV/epoch)', ...
           fastif(icacomp==0,'(uV/epoch)','std. dev./epoch') ] }, ...
