...
cb_compthresh = [ '  posthresh = get( findobj(''parent'', gcbf, ''tag'', ''threshpos''), ''string'' );' ...
				  ...
				  ', elecrange, negthresh, posthresh, startime/1000, endtime/1000, rejstatus, 0,''' tmp_comall ''');' ...
				  'EEG = h(LASTCOM, EEG);' ...
...
cb_reject =      [ 'set( findobj(''parent'', gcbf, ''tag'', ''rejstatus''), ''value'', 3);' ... % force status to 3 
				    checkstatus ...
				   '[EEG LASTCOM] = eeg_rejsuperpose(EEG,' int2str(icacomp) ',1,1,1,1,1,1,1); EEG = h(LASTCOM, EEG);' ...
				   '[EEG LASTCOM] = pop_rejepoch( EEG, EEG.reject.rejglobal, 1);' ...
				   'if ~isempty(LASTCOM), ' ...
				   '   EEG = h(LASTCOM, EEG); [ALLEEG EEG CURRENTSET LASTCOM] = pop_newset(ALLEEG, EEG, CURRENTSET); h(LASTCOM);' ...
...
lisboxoptions = { 'string', [ 'Show only the new trials marked for rejection by the measure selected above|' ...
	...
	{ } { 'Style', 'pushbutton', 'string', fastif(icacomp,'Scroll Data','Scroll Comp. Acts.'), 'callback', cb_manual }, ...
