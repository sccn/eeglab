...
function [logersp, logfreqs] = std_readersp(ALLEEG, abset, comp);
...
for k = 1: length(abset)    
	...
	try
        ...
        warndlg2(['std_readersp: file '  ALLEEG(abset(k)).etc.icaersp ' was not found in path ' ALLEEG(abset(k)).filepath], 'Abort - std_readersp' ); 
        return;
	end
end	
