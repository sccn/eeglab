...
%   'timeunit'     - [float] time unit in second.
%   'elec'         - ['on'|'off'] include electrodes names or number. 
%                    Default 'on'.
%   'transpose'    - ['on'|'off'] using 'off', data for each electrode is 
%                    written in one row. Using 'on', data is written in
%                    columns.
%   'erp'          - ['on'|'off'] export ERP instead of raw data. Default 'off'.
%   'expr'         - [string] evaluate epxression on data. The expression must 
%                    contain a 'x' variable representing the 2-D or 3-D data.
%   'precision'    - [float] number of significant digits in output. 
%                    Default is 4. 
% 
% Outputs:
...
if nargin < 2
   ...
   uilist = { { 'style' 'text' 'string' 'Output file name' }, ...
              ...
              { 'style' 'text' 'string' 'Check to export ICA activities instead of EEG data:' }, ...
              { 'style' 'checkbox' 'string' '' }, { },{ },  ...
              { 'style' 'text' 'string' 'Check to export ERP instead of activity:' }, ...
              { 'style' 'checkbox' 'string' '' }, { }, { }, ...
              { 'style' 'text' 'string' 'Check to transpose matrix (elec -> rows):' }, ...
              { 'style' 'checkbox' 'string' '' }, { }, { }, ...
              { 'style' 'text' 'string' 'Check to export channel labels/comp number:' }, ...
              { 'style' 'checkbox' 'string' '' 'value' 1 }, { }, { }, ...
              { 'style' 'text' 'string' 'Check to export time values:' }, ...
              { 'style' 'checkbox' 'string' '' 'value' 1 }, ...
              { 'style' 'text' 'string' 'Units (s)' }, { 'style' 'edit' 'string' '1E-3' }, ...
              { 'style' 'text' 'string' 'Number of significan digits to output:' }, ...
              { 'style' 'edit' 'string' '4' }, { }, { }, ...
              { 'style' 'text' 'string' 'Evaluate expression on data (see ''expr'' help):'} , ...
              { 'style' 'edit' 'string' '' } { } { } };
