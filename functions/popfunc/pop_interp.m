...
function [EEG com] = pop_interp(EEG, bad_elec, method)
    ...
    if nargin < 2
        ...
        cb_nondat   = [ '[chanlisttmp chanliststr] = pop_chansel( { EEG.chaninfo.nodatchans.labels } );' ...
        ...
        cb_otherdat = [ 'tmpanswer = inputdlg2({ ''Dataset index'' }, ''Choose dataset'', 1, { '''' });' ...
                      ...
                      '           tmpchanlist = setdiff( { ALLEEG(tmpanswernum).chanlocs.labels }, { EEG.chanlocs.labels } );' ...
    ...
    elseif isstr(EEG)
        ...
        if strcmpi(command, 'nondatchan')
            global EEG;
            [chanlisttmp chanliststr] = pop_chansel( { EEG.chaninfo.nodatchans.labels } );
            if ~isempty(chanlisttmp),
                ...
                if ~isempty(tmpanswernum),
                    if tmpanswernum > 0 & tmpanswernum < length(ALLEEG),
                        ...
                        if strcmpi(command, 'selectchan')
                            chanlist = pop_chansel( { TMPEEG.chanlocs.labels } );
                        else
                            chanlist = 1:length(TMPEEG.chanlocs); % use all channels
                        end
                        
                        % look at what new channels are selected
                        [tmpchanlist chaninds] = setdiff( { TMPEEG.chanlocs(chanlist).labels }, { EEG.chanlocs.labels } );
                        if ~isempty(tmpchanlist),
