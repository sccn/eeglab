...
if nargin < 3
    ...
    uiconvert = { ...
        ...
        {  }, ...
        { 'Style', 'pushbutton', 'string', 'xyz -> polar & sph.', 'callback', 'pop_chanedit(gcbf, [], ''convert'', {''cart2all'' ''gui''});' }, ...
        { 'Style', 'pushbutton', 'string', 'sph. -> polar & xyz', 'callback', 'pop_chanedit(gcbf, [], ''convert'', {''sph2all''  ''gui''});' }, ...
        { 'Style', 'pushbutton', 'string', 'polar -> sph. & xyz', 'callback', 'pop_chanedit(gcbf, [], ''convert'', {''topo2all'' ''gui''});' }, ...
        {  }, ...
    ...
    for curfield = 1:2:length(args)
        switch lower(args{curfield})
            ...
            case 'setref'
                if ~isempty(fig)
                    args{curfield+1} = inputdlg2({'Channel indices' 'Ref (e.g. Cz)' }, ...
