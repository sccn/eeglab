% pop_readlocs() - load a EGI EEG file (pop out window if no arguments).
%
% Usage:
%   >> EEG = pop_readlocs;             % a window pops up
%   >> EEG = pop_readlocs( filename, 'key', val, ...);
%
% Inputs:
%   filename       - Electrode location file name
%   'key',val      - same as readlocs()
% 
...
if nargin < 1 
   ...
   indexfiletype = strmatch(filetype, listtype); 
   ...
   formatcom = [  ...
         ...
         '   end;' ...
         'end;' ...
         'set(findobj(gcf, ''tag'', ''headlines''), ''string'', int2str(tmpformat{2}(indexformat)));' ...
         'eval(get(findobj(gcf, ''tag'', ''headlines''), ''callback''));' ...
         'clear tmpindex indexformat tmpformat tmpobj;' ];
         
   headercom = [ ...
         'tmpheader = str2num(get(findobj(gcf, ''tag'', ''headlines''), ''string''))+1;' ...
         'for tmpindex=1:' int2str(length(nbcols)) ...
         '   tmpobj = findobj(gcf, ''tag'', [ ''text'' int2str(tmpindex) ]);' ...
         '   tmpstr = get(tmpobj, ''userdata'');' ...
         '   set(tmpobj, ''string'', tmpstr(tmpheader:min(tmpheader+9, size(tmpstr,1)),:));' ...
         'end;' ...
