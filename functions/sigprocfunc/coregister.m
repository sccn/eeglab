...
function [elec1, transf] = warp_chans(elec1, elec2, chanlist, warpmethod)
    ...
    olddir = pwd;
    try,
        tmp = which('electroderealign'); tmp = fullfile(fileparts(tmp), 'private');
        cd(tmp);
...
% ELECTRODENORMALIZE rotates and translates electrode positions to standard
% or averaged standard electrode positions.
%
% Use as
%   [elec] = electrodenormalize(cfg)
% The configuration can contain the following options
%   cfg.channel        = cell-array, see CHANNELSELECTION
%   cfg.casesensitive  = 'yes' (default) or 'no' determines whether string comparisons
%                        of electrode labels are case sensitive or not
%
% Different methods are implemented. You can apply a spatial deformation
% (i.e. 'warp') that minimizes the distance between the electrodes and the
% averaged standard. Alternatively, you can apply a rigid body realignment
% based on three fiducial locations.
%
%   cfg.method         = 'warp' or 'realignfiducial'
%   cfg.fiducial       = cell-array with the name of three fiducials used
%                        for realigning, default is {'nasion', 'lpa', 'rpa'}
%
% The electrode set of interest that will be normalized is specified as
%   cfg.elecfile       = string with filename, or alternatively
%   cfg.elec           = structure with electrode definition
%
% The electrode set(s) that serve as the standard can be specified either
% as electrode structures (i.e. when they are already read in memory) or as
% electrode files.
%
...
% set the defaults
if ~isfield(cfg, 'method'),   cfg.method = 'unknown';    end
if ~isfield(cfg, 'channel'),  cfg.channel = 'all';       end
if ~isfield(cfg, 'feedback'), cfg.feedback = 'no';       end
if ~isfield(cfg, 'plot3d'),   cfg.plot3d = 'no';         end
...
if any(strcmp(cfg.method, {'rigidbody', 'globalrescale', 'traditional', 'nonlin1', 'nonlin2', 'nonlin3', 'nonlin4', 'nonlin5'}))
  ...
  [norm.pnt, norm.m] = warp_pnt(elec.pnt, avg, cfg.method);
