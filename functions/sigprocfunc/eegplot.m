...
%    'movebutton' - ['on'|'off'] Display the move buttons (does not work well, if
%                   children)
...
if ~isstr(data) % If NOT a 'noui' call or a callback from uicontrols
   ...
   for index=1:length(gfields)
      ...
      case {'spacing', 'srate' 'eloc_file' 'winlength' 'position' 'title' ...
               ...
               'freqlimits' 'submean' 'children' 'movebutton' 'limits' 'dispchans' 'wincolor' ...
   ...
   switch lower(g.movebutton)
	   case { 'on' 'off' };
	   otherwise disp('Error: movebutton must be either ''on'' or ''off'''); return;
   end;	
