...
%
% Revision 1.119  2007/05/07 23:27:52  arno
% motioncapture option
%
...
if ~isstr(data) % If NOT a 'noui' call or a callback from uicontrols
   ...
   try, g.movebutton;		catch, g.movebutton = 'on';  end;
   try, g.motioncapture;    catch, g.motioncapture = 'off'; end;
   ...
   for index=1:length(gfields)
      ...
      case {'spacing', 'srate' 'eloc_file' 'winlength' 'position' 'title' ...
               ...
               'ploteventdur' 'butlabel' 'scale' 'events' 'data2' 'plotdata2' 'movebutton' },;
...
% ESpacing buttons: + -
  ...
  commandpush = ['ax1 = findobj(''tag'',''backeeg'',''parent'',gcbf);' ... 
             ...
             '     highlim = round(g.winlength*g.srate);' ...
  			 '  end;' ... 
             '  if strcmpi(g.motioncapture, ''on''),' ... % motion capture call
             '     show_mocap(EEG, tmppos+lowlim);' ...
             '  elseif (tmppos(1) >= 0) & (tmppos(1) <= highlim),' ...
  ...
  commandrelease = ['ax1 = findobj(''tag'',''backeeg'',''p arent'',gcbf);' ... 
			 ...
			 'if ~isempty(g.winrej) & ~strcmpi(g.motioncapture, ''on''),' ... % motion capture call
