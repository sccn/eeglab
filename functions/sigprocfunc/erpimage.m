...
%              to -1 in icadefs.m {default no ERP plotted}
...
%
% 2009/09/25 David Groppe, Kutaslab
% Added:
% -option avg_type (thanks to Jeffrey S. Johnson for the idea)
% -option for plotting median/tertile/quartile split ERPs (this also
% changed the color of ERPs)
% -option erp_grid
% -erps are plot in front of voltage=0 and time=0 lines
% -Added y-axis ticks for ERP
% -option cbar_title
% -Removed xticks for colorbar
% -option img_trialax_label
% -option img_trialax_ticks
% -option filt
% -option baseline
% -max and min ERP voltage limits can now be independently specified
%  (before max=max(abs([minerp maxerp])); and min=-max)
% -sortvar limits can be specified (affects visualization, not smoothing)
%
...
if Erpflag == YES
    if exist('erpalpha')
        ...
        for ploop=1:erp_ptiles,
            ...
            erp(ploop,:)=nan_mean(fastif(length(tsurdata) > 0, ...
                tsurdata(:,ptile_trials)',urdata(:,ptile_trials)'));
        end
       % else
            %orig line
...
if Erpflag == YES & strcmpi(noshow, 'no')
    ...
    if Erpstdflag == YES
        if Showwin
            tmph = plot1trace(ax2,times,erp,limit, [], stdev,times(winlocs),erp_grid); % plot ERP +/-stdev
        else
            tmph = plot1trace(ax2,times,erp,limit, [], stdev,[],erp_grid); % plot ERP +/-stdev
        end
    elseif ~isempty('erpsig')
        erpsig = [erpsig;-1*erpsig];
        if Showwin
            tmph = plot1trace(ax2,times,erp,limit,erpsig,[],times(winlocs),erp_grid); % plot ERP and 0+/-alpha threshold
        else
            tmph = plot1trace(ax2,times,erp,limit,erpsig,[],[],erp_grid); % plot ERP and 0+/-alpha threshold
        end
    else % plot ERP alone - no significance or std dev plotted
        if Showwin
            tmph = plot1trace(ax2,times,erp,limit,[],[],times(winlocs),erp_grid); % plot ERP alone
        else
            tmph = plot1trace(ax2,times,erp,limit,[],[],[],erp_grid); % plot ERP alone
        end
    end;
...
%% %%%%%%%%%%%%%%%%% function plot1trace() %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
function [plot_handle] = plot1trace(ax,times,trace,axlimits,signif,stdev,winlocs,erp_grid)
%function [plot_handle] = plot1trace(ax,times,trace,axlimits,signif,stdev,winlocs,erp_grid)
...
linecolor={[1 0 .5],[.5 0 1],[0 .5 1],[0 1 .5]};
...
if ~isempty(axlimits) && sum(isnan(axlimits))==0
    ...
    %y-ticks
    shrunk_ylimits=round(axlimits(3:4)*.8);
...
kids=[plot_handle setdiff(kids,plot_handle)]';
