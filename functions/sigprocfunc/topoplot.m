...
try,
    if strcmpi(STYLE,'blank') % if mark-selected-channel-locations mode
        for kk = 1:length(1:length(x))
            ...
            elseif Values(kk) == 2
                hp2 = plot3(y(kk),x(kk),ELECTRODE_HEIGHT,EMARKER,'Color', [1 0.7 0.1], 'markersize', EMARKERSIZE1CHAN);
            elseif Values(kk) == 1
                hp2 = plot3(y(kk),x(kk),ELECTRODE_HEIGHT,EMARKER,'Color', [1 1 0], 'markersize', EMARKERSIZE1CHAN);
