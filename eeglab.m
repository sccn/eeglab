...
second_m = uimenu( W_MAIN, 'Label', 'Edit');
	...
	uimenu( second_m, 'Label', 'Copy current dataset'  , 'CallBack', [ check      '[ALLEEG LASTCOM] = pop_copyset(ALLEEG); h(LASTCOM); eeglab(''redraw'');' e_hist], 'Separator', 'on');
...
third_m = uimenu( W_MAIN, 'Label', 'Plot');
	...
	uimenu( third_m, 'Label', 'Channel spectra and maps' , 'CallBack', [ checkicaplot   'LASTCOM = pop_spectopo(EEG, 0);' e_hist]);
