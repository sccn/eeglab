...
catchstrs.new_non_empty          = e_newset;
	...
	uimenu( filter_m, 'Label', 'Original EEGLAB FIR filtering method'   , 'CallBack', cb_eegfilt);
...
while( index <= MAX_SET)
	...
	try, ALLEEG(index).data;
		if ~isempty( ALLEEG(index).data)
            ...
            cb_retrieve = [ '[ALLEEG EEG CURRENTSET LASTCOM] = pop_newset(ALLEEG, EEG, CURRENTSET, ''retrieve'', ' int2str(index) ');' ...
                            'if CURRENTSTUDY, LASTCOM = [ ''CURRENTSTUDY = 0; '' LASTCOM ]; eval(LASTCOM); eegh(LASTCOM); end;' ...
                            'eeglab(''redraw'');' ];
...
% -----------------------------------
if index ~= 0
    cb_select = [ '[ALLEEG EEG CURRENTSET LASTCOM] = pop_newset(ALLEEG, EEG, CURRENTSET, ''retrieve'', []);' ...
