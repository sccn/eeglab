...
third_m = uimenu( W_MAIN, 'Label', 'Plot');
	...
	uimenu( third_m, 'Label', 'Channel statistics'       , 'CallBack', [ check          'LASTCOM = pop_signalstat(EEG, 1);' e_hist], 'enable', fastif(exist('weibstat'), 'on', 'off'));
	...
	uimenu( third_m, 'Label', 'Component statistics'     , 'CallBack', [ check          'LASTCOM = pop_signalstat(EEG, 0);' e_hist], 'enable', fastif(exist('weibstat'), 'on', 'off'));
