...
first_m = uimenu( W_MAIN, 'Label', 'File');
	...
	uimenu( importevent, 'Label', 'Import events from data channel'          , 'CallBack', [ check   '[EEG LASTCOM]= pop_chanevent(EEG);' e_store]); 
...
second_m = uimenu( W_MAIN, 'Label', 'Edit');
	...
	uimenu( second_m, 'Label', 'Channel locations'   , 'CallBack', [ '[TMPCHAN LASTCOM] =pop_chanedit(EEG.chanlocs); if ~isempty(LASTCOM), EEG.chanlocs = TMPCHAN; clear TMPCHAN;' storecall 'end; eeglab(''redraw'');']);
...
third_m = uimenu( W_MAIN, 'Label', 'Plot');
	...
	uimenu( third_m, 'Label', 'Channel spectra and maps' , 'CallBack', [ checkplot      'LASTCOM = pop_spectopo(EEG);' e_hist]);
	uimenu( third_m, 'Label', 'Channel ERP image'        , 'CallBack', [ checkepoch     'LASTCOM = pop_erpimage(EEG, 1, h(''find'',''pop_erpimage''));' e_hist]);
	...
	uimenu( third_m, 'Label', 'Component ERP image'      , 'CallBack', [ checkepochica  'LASTCOM = pop_erpimage(EEG, 0, h(''find'',''pop_erpimage''));' e_hist]);
