...
n=size(ALLEEG,2); % number of total datasets
...
tot_ics=length(ALLEEG(n_tmp).icawinv); %total number of ICs in template dataset
...
%if STUDY is not pre clustered it is not possible to create a cluster using "clname"

if isempty(clname)

elseif isempty(STUDY.preclust.erpclusttimes) && isempty(STUDY.preclust.specclustfreqs)

    fprintf('Error: STUDY is not pre-clustered. If you want to save CORRMAP cluster, please pre-cluster the STUDY.\n');
    fprintf('see >> help corrmap. \n');
    return
end
...
chan=ALLEEG(n_tmp).chanlocs; % channel information

% aux=1:n;

for ii=1:n; %auxiliary variable

    comp{ii}=ALLEEG(ii).icawinv; %creating cell with all datasets
    ics_set(ii)=size(ALLEEG(ii).icawinv,2); %number of ICs in each dataset
    names{ii}=ALLEEG(ii).setname; %creating cell with all dataset names

    %checking if all datasets have the same number of channels
    ch(ii)=ALLEEG(ii).nbchan;
...
if length(ch_aux)==0 || length(ch_aux)<n-1
    ...
    if length(chanlocs)==0
        fprintf('Error: Datasets do not have the same number of channels.\nPlease re-run CORRMAP and insert channels location file.\n');
        fprintf('>> see help corrmap and help interp_chan. \n');
        return
    else
        ...
        fprintf('>> \n');
        clear comp
        %         comp1={ALLEEG(aux).icawinv}; %creating cell with all datasets
        ...
        nbDatasets = length(ALLEEG);
        chanLabels = {};
        nbChans = 0;
        for i=1:nbDatasets
            for j=1:length({ALLEEG(i).chanlocs.labels})
                nbChans = nbChans+1;
                chanLabels(nbChans) = {ALLEEG(i).chanlocs(j).labels};
            end
        end
        % Get unique labels
        ...
        chan  = CHANS;
        for ind = 1:length(ALLEEG)
            TMPEEG = eeg_emptyset;
            TMPEEG.data       = ALLEEG(ind).icawinv;
            TMPEEG.chanlocs   = ALLEEG(ind).chanlocs(ALLEEG(ind).icachansind);
            TMPEEG.nbchan     = length(ALLEEG(ind).icachansind);
            TMPEEG.trials     = 1;
            TMPEEG.pnts       = size(TMPEEG.data,2);
            TMPEEG.xmin       = 0;
            TMPEEG.srate      = ALLEEG(ind).srate;
            TMPEEG.xmax       = (TMPEEG.pnts-1)/TMPEEG.srate;
            TMPEEG.icawinv    = [];
            TMPEEG = eeg_interp(TMPEEG, CHANS);
            comp{ind} = TMPEEG.data;
        end;
        %[comp,CHANS]=interp_chan(ALLEEG,chanlocs); %function to create cell with new datasets
...
if strcmp(badcomps,'yes')
    ...
    for i=1:length(CORRMAP.output.sets{2})
        ...
        if isempty(ALLEEG(a).badcomps) % EEG.badcomponents is empty
            ALLEEG(a).badcomps(i)=CORRMAP.output.ics{2}(i);
        else
            nbc=length(ALLEEG(a).badcomps); %number of bad components already stored
            ALLEEG(a).badcomps(nbc+1)=CORRMAP.output.ics{2}(i);
        end

    end

    for jj=1:length(CORRMAP.datasets.index)
        aux_ind=find(ALLEEG(jj).badcomps==0);
        ALLEEG(jj).badcomps(aux_ind)=[];
        CURRENTSTUDY = 0;EEG=ALLEEG(jj); [ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, jj , 'overwrite','on', 'study',1);
