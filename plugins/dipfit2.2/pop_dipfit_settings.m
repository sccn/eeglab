...
% Revision 1.33  2007/08/16 00:41:26  arno
% remove debug message
%
% Revision 1.32  2007/08/16 00:40:19  arno
% debug last modifcations
%
...
% CTF implementation
%
% Revision 1.28  2006/11/06 21:36:51  arno
% default coregistration matrix for MNI brain
%
...
if nargin < 2
    ...
    if isfield(EEG.chaninfo, 'filename')
        ...
        if ~isempty(findstr(lower(EEG.chaninfo.filename), 'standard_1005')),        valmodel = 2; coregvals = num2str(template_models{valmodel}.coord_transform); end;
    ...
    headstr = 'Head model file';
    mristr  = 'MRI file';
    chanstr = 'Model template channel locations file';
    setmodel = [ 'tmpdat = get(gcbf, ''userdata'');' ...
                 'tmpval = get(gcbo, ''value'');' ...
                 'set(findobj(gcbf, ''tag'', ''model''), ''string'', char(tmpdat(tmpval).hdmfile));' ...
                 'set(findobj(gcbf, ''tag'', ''coord''), ''value'' , fastif(strcmpi(tmpdat(tmpval).coordformat,''MNI''),2,fastif(strcmpi(tmpdat(tmpval).coordformat,''CTF''),3,1)));' ...
                 'set(findobj(gcbf, ''tag'', ''mri''  ), ''string'', char(tmpdat(tmpval).mrifile));' ...
                 'set(findobj(gcbf, ''tag'', ''meg''), ''string'', char(tmpdat(tmpval).chanfile));' ...
                 ...
                 'end;' ...
                 'if tmpval == 3,' ...
                 '  set(findobj(gcbf, ''tag'', ''headstr''), ''string'', ''Subject CTF head model file (default.htm)'');' ...
                 '  set(findobj(gcbf, ''tag'', ''mristr''),  ''string'', ''Subject MRI (coregistered with CTF head)'');' ...
                 '  set(findobj(gcbf, ''tag'', ''chanstr''), ''string'', ''CTF Res4 file'');' ...
                 '  set(findobj(gcbf, ''tag'', ''manualcoreg''), ''enable'', ''off'');' ...
                 '  set(findobj(gcbf, ''userdata'', ''coreg''), ''enable'', ''off'');' ...
                 'else,' ...
                 '  set(findobj(gcbf, ''tag'', ''headstr''), ''string'', ''' headstr ''');' ...
                 '  set(findobj(gcbf, ''tag'', ''mristr''),  ''string'', ''' mristr ''');' ...
                 '  set(findobj(gcbf, ''tag'', ''chanstr''), ''string'', ''' chanstr ''');' ...
                 '  set(findobj(gcbf, ''tag'', ''manualcoreg''), ''enable'', ''on'');' ...
                 '  set(findobj(gcbf, ''userdata'', ''coreg''), ''enable'', ''on'');' ...
                 'end;' ];
        
    elements  = { ...
        { 'style' 'text'        'string'  [ 'Head model (click to select)' 10 '' ] } ...
        { 'style' 'listbox'     'string'  'Spherical Four-Shell (BESA) | Boundary Element Model (MNI) | CTF MEG | Custom Head' ... 
                                'callback' setmodel 'value' valmodel } { } ...
        { 'style' 'text'        'string' headstr 'tag' 'headstr' } ...
        { 'style' 'edit'        'string' char(template_models(valmodel).hdmfile)  'tag'      'model' 'userdata' 'editable' 'enable' 'off'} ...
        ...
        { 'style' 'popupmenu'   'string' 'spherical (head radius 85 mm)|MNI|CTF' 'tag' 'coord' ...
          'value' fastif(strcmpi(template_models(valmodel).coordformat,'MNI'),2,1)  'userdata' 'editable' 'enable' 'off'} ...
        { 'style' 'text'        'string' 'Click to select' } { } ...
        { 'style' 'text'        'string' mristr 'tag' 'mristr' } ...
        { 'style' 'edit'        'string' char(template_models(valmodel).mrifile) 'tag' 'mri' } ...
        { 'style' 'pushbutton'  'string' 'Browse'       'callback' commandload3 } ...
        { 'style' 'pushbutton'  'string' 'Help'         'callback' comhelp3 } ...
        { 'style' 'text'        'string' chanstr, 'tag', 'chanstr' } ...
        { 'style' 'edit'        'string' char(template_models(valmodel).chanfile) 'tag' 'meg'  'userdata' 'editable' 'enable' 'off'} ...
        ...
        { 'style' 'edit'        'string' coregvals 'tag' 'coregtext' 'userdata' 'coreg' } ...
        { 'style' 'pushbutton'  'string' 'Manual Co-Reg.' 'tag' 'manualcoreg' 'callback' cb_selectcoreg 'userdata' { EEG.chanlocs EEG.chaninfo } } ... 
        { 'style' 'checkbox'    'string' 'No Co-Reg.'    'tag' 'coregcheckbox' 'value' nocoregvalue  'userdata' 'coreg' } ... 
