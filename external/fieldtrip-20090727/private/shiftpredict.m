...
switch cfg.method
case {'abscoh', 'imagcoh', 'absimagcoh'}
  ...
  for i=1:nsgn
    for j=1:nfrq
      dat(i,:,j) = dat(i,:,j) ./ sqrt(pow_tot(i,j)*ntrl*trltapcnt(1));
    end
  end
...
case 1
  ...
  for j=1:nsgn
    switch method
    case 'abscoh'
      coh(i,j,:) = abs(     sum(datref(i,:,:) .* conj(dat(j,:,:)), 2));
    case 'imagcoh'
      coh(i,j,:) =     imag(sum(datref(i,:,:) .* conj(dat(j,:,:)), 2));
    case 'absimagcoh'
      coh(i,j,:) = abs(imag(sum(datref(i,:,:) .* conj(dat(j,:,:)), 2)));
    case 'amplcorr'
      coh(i,j,:) =          sum(datref(i,:,:) .* conj(dat(j,:,:)), 2);
    case 'absamplcorr'
      coh(i,j,:) = abs(     sum(datref(i,:,:) .* conj(dat(j,:,:)), 2));
    otherwise
      error('unsupported method');
