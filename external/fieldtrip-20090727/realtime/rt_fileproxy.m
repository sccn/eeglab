...
while true
  ...
  if newsamples>=minblocksmp
    ...
    % it only makes sense to read those events associated with this
    % currently processed data. I.e., future events are ill-defined...
    % FIXME: read_event/write_event not yet stable: slow and crashes...
    if strcmp(cfg.readevent,'yes')
      evt = read_event(cfg.source.datafile,'header',hdr,'minsample',begsample,'maxsample',endsample);
    end
